...
const BlogPage = () => {
    ...
    const partnerPosts = posts.filter(post => post.category?.toLowerCase() === "partners").slice(0, 3);
    ...
    return (
        <>
            ...
            {majorBlog && (
                <div
                    ...
                    {/* Major Blog Section */}
                    <div className="w-full lg:w-6/12 p-6 rounded-xl" style={{boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px"}}>
                    ...
                    <p className=" mb-4 line-clamp-2 lg:line-clamp-1">{majorBlog.description}</p>
                    <div className="flex items-center justify-between ">
                        ...
                        <span className={"text-gray-800"}>{new Date(majorBlog.publishedAt).toLocaleDateString()}</span>
                    </div>
                </div>
            {/* Latest Blog Updates Section */}
                <div className=" w-full lg:w-6/12 bg-white p-6 rounded-xl" style={{boxShadow: "rgba(99, 99, 99, 0.2) 0px 2px 8px 0px"}}>
                                    ...
                                    <div key={index} className="flex flex-col lg:flex-row items-center">
                                        <div className={"w-full my-3 lg:w-6/12"}>
                                            ...
                                            <Link to={`/blog/${post.slug.current}`}>
                                                <h4 className="font-bold my-2 hover:underline">{post.title}</h4>
                                            </Link>
        ...
        {/* Categories Tabs - Scrollable on Mobile */}
                ...
                <div className="my-12 w-10/12 mx-auto">
                    <h2 className="text-3xl font-bold mb-6">Promote</h2>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        {promotePosts.length > 0 && (
                            ...
                            <p className="text-gray-600 mb-4 line-clamp-3">{promotePosts[0].description}</p>
                            <div className="flex justify-between items-center text-sm text-gray-500">
                                <span>Pitchle Team</span>
            ...
            <div className="my-12 w-10/12 mx-auto">
                <h2 className="text-3xl font-bold mb-6">Sell & Transfer</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {sellPosts.length > 0 && (
                        <div className="md:col-span-1 bg-white p-6 rounded-xl shadow-md">
                            <img src={sellPosts[0].image?.asset?.url} alt="Promote Blog"
                                 className="w-full rounded-lg mb-4"/>
                            ...
                            <p className="text-gray-600 mb-4 line-clamp-3">{sellPosts[0].description}</p>
                            <div className="flex justify-between items-center text-sm text-gray-500">
                                <span>Pitchle Team</span>
                                <span>{new Date(promotePosts[0].publishedAt).toLocaleDateString()}</span>
                            </div>
                ...
                <div className="flex justify-center mt-6">
                    <button
                        className="px-6 py-3 bg-blue-500 text-white rounded-full shadow-lg hover:bg-green-600 transition">
                        View All →
                    </button>
                </div>
            </div>
